class TeacherProfilesController < ProfilesController
  def index
    ...
    if params[:ratings_option] == "order_by_reviews"
      @teacher_profiles = TeacherProfile.send(params[:ratings_option]).chain_scopes(params).include_associated.paginate(page: params[:page], per_page: PER_PAGE)
    elsif params[:ratings_option] == "order_by_average_rating"
      @teacher_profiles = (TeacherProfile.send(params[:ratings_option]).chain_scopes(params).include_associated + 
                          TeacherProfile.zero_review_records.chain_scopes(params)).include_associated.paginate(page: params[:page], per_page: PER_PAGE)
    else
